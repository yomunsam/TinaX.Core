...
namespace CatLib.Container
{
    ...
    public class Container : IContainer
    {
        ...
        protected virtual object ResolveAttrPrimitive(Bindable makeServiceBindData, string service, PropertyInfo baseParam)
        {
            ...
            if (CanProjectInjectSkip(baseParam))
                return skipped;
        ...
        protected virtual void AttributeInject(Bindable makeServiceBindData, object makeServiceInstance)
        {
            ...
            Type makeServiceInstanceType = GetType(ref makeServiceInstance);
            foreach (var property in makeServiceInstanceType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                if (!CanPropertyInject(property))
                {
                    continue;
                }
                ...
                if (!CanInject(property.PropertyType, instance))
                {
                    throw new UnresolvableException(
                        $"[{makeServiceBindData.Service}]({makeServiceInstanceType}) Attr inject type must be [{property.PropertyType}] , But instance is [{instance?.GetType()}] , Make service is [{needService}].");
                }
        ...
        /// <returns></returns>
        protected virtual Type GetType(ref object sourceObject)
        ...
        /// <returns></returns>
        protected virtual bool CanPropertyInject(PropertyInfo property)
        {
            return property.CanWrite && property.IsDefined(typeof(CatLib.Container.InjectAttribute), true);
        }
        ...
        /// <param name="property"></param>
        /// <returns></returns>
        protected virtual bool CanProjectInjectSkip(PropertyInfo property)
        {
            var attribute = property.GetCustomAttribute<CatLib.Container.InjectAttribute>(true);
            if (attribute == null)
                return true;
            else
